...
class Distribution:
    ...
    def dropsku(self):
        ...
        totalym = list(df9["YearMonth"].unique())
        # recent 4 months
        recentym = totalym[-5:-1]
        
        littlesku = []
        for sku in totalsku:
            skudf = df9[df9["SKU_Eng"]==sku]
            # list up SKUs that only have same or less than recent 4 months of data
            # check if elements of recentym are in skudf yearmonth
            recentcheck = all(m in list(skudf["YearMonth"].unique()) for m in recentym)
            if(recentcheck):
                pass
            else:
        ...
        # remove last yearmonth rows
        # due to the fact that it includes only small amount of data
        lastym = totalym[-1]
        df10 = df10[df10["YearMonth"] != lastym]
