...
class DistGraph:
    ...
    def __init__(self):
        ...
        self.keysku = ["Tylenol 500MG BLST 10S KR", "Tylenol Cold-S 325mg 10T'16",
                       "Children's Tylenol ODG 160mg 12T",
        ...
        self.cdmap = {"Tylenol Cold-S 325mg 10T'16": "#FFA15A",
                      ...
                      "TYLENOL 8HR ER 650MG BLST 6S KR": "#FF7D6B",
                      "Tylenol 32MG/ML 100ml Susp.DF_R'14_v2.": "#FF7D6B",
                      "Tylenol 500MG BLST 10S KR (8806469025729)": "#EF553B",
                      "Children's Tylenol ODG 160mg 12T": "#FF6692",
                      ...
                      "Listerine Freshburst 750ml_KR (BD)": "#72B7B2",
                      "Listerine Freshburst 250ml_CR (KR0318)": "#72B7B2",
                      "Listerine Citrus 250ml_KR(BD)": "#72B7B2",
                      "Listerine Citrus 750ml_KR (BD)": "#72B7B2",
                      "Listerine Cool Mint Mild 250mlJN(KR0518)": "#72B7B2",
                      "Listerine Cool Mint Mild 750mlCR(KR0318)": "#72B7B2",
                      "Listerine Coolmint 100ml_KR (A)": "#72B7B2",
                      "Listerine Coolmint 250ml_KR(BD)": "#72B7B2",
                      "Listerine Green Tea 750ml_KR (A)": "#72B7B2",
                      "Listerine Natural GT Mild 100ml": "#72B7B2",
                      "Listerine Natural GT Mild250mlCR(KR0318)": "#72B7B2",
                      "Listerine Total Care Plus750mlCR(KR1118)": "#72B7B2",
                      "Listerine Total Care Zero 750ml_POP(D)": "#72B7B2",
                      "Rogaine Foam 60ml_3packs_Male": "#636EFA", 
                      "Rogaine Foam 60ml_Male": "#636EFA"}
        # Area orders
        self.arealist= ["서울시","경기도","강원도","충청북도","충청남도","경상북도","경상남도","전라북도","전라남도","제주도"]
        # need to edit start and end month/week according to data file
        self.figlinems = "2021-01-01"
        self.figlineme = "2022-12-01"
        self.figlinews = 1
        self.figlinewe = 52
    ...
    def monthlyfig(self):
        ...
        fig1 = px.line(mdf1, x="YYYYMM", y="MonthlyPCT", color="SKU_Eng",
                       ...
                       height=550, width=1250, markers=True,
        ...
        linetyc = [dict(type="line", xref="x", yref="y",
                        y0=96, y1=96, x0=self.figlinems, x1=self.figlineme,
                        line=dict(color="MediumPurple", dash="dot"))]
    ...
    def weeklyfig(self):
        ...
        fig2 = px.line(wdf2, x="VisitWeek", y="WeeklyPCT", color="SKU_Eng",
                       ...
                       color_discrete_map=self.cdmap,
                       height=550, width=1250, markers=False,
        ...
        # Add buttons that add horizontal target lines by Brand
        linetyp = [dict(type="line", xref="x", yref="y",
                        y0=92, y1=92, x0=self.figlinews, x1=self.figlinewe,
                        line=dict(color="MediumPurple", dash="dot"))]
        linetyc = [dict(type="line", xref="x", yref="y",
        ...
        if (period=="M"):
            ...
            if (str(pharm)=="Area"):
                ...
                fig3 = px.line(df4, x="YYYYMM", y=str("M"+str(pharm)+"PCT"), color="SKU_Eng",
                               ...
                               color_discrete_map=self.cdmap,
                               height=fheight, width=1250, markers=True,
                               title=str("Monthly Distribution Rate(%) by "+str(pharm)))
                
                # monthly heatmap by area
                fig4 = self.mpctheatmap(df4, pharm, arealst)
            ...
            if (str(pharm)=="Area"):
                ...
                fig3 = px.line(df4, x="VisitWeek", y=str("W"+str(pharm)+"PCT"), color="SKU_Eng",
                               ...
                               color_discrete_map=self.cdmap,
                               height=fheight, width=1250, markers=False,
    ...
    def mpctheatmap(self, mydf, mypharm, myarea):
        
        # from same pharmacy filter, only keep last YYYYMM row of each SKU
        mydf.sort_values(by=[str(mypharm),"SKU_Eng","YYYYMM"], inplace=True)
        mydf.drop_duplicates(subset=[str(mypharm),"SKU_Eng"], keep="last", inplace=True)
        selected = mydf[["SKU_Eng", str(mypharm), "MAreaPCT"]]
        widedf = selected.pivot(index="SKU_Eng", columns=str(mypharm), values="MAreaPCT")
        
        # only leave key SKU
        mykeys = self.keysku
        keydf = widedf.loc[widedf.index.isin(mykeys)]
        
        # sort sku order
        keydf = keydf.reindex(index = mykeys)
        # sort area order
        keydf = keydf.loc[:, myarea]
        
        # heatmap
        myfig = px.imshow(keydf, text_auto=".2f", color_continuous_scale='Blackbody',
                          height=850, width=1250, aspect="auto",
                          title=str("Heatmap: Key SKU Distribution Rate(%) by "+str(mypharm)))
        
        # save html file in static folder
        myhtml = str("./static/dashboard/"+"heatmap"+str(mypharm)+".html")
        myfig.write_html(myhtml)
